...
type subConn struct {
	...
	si      serverInfo //cpu的相关信息
	// effective weight
	//有效权重 ，初始值为weight，当发生错误就减小ewt的值，此后有新的请求过来时，逐渐增加ewt的值，又恢复到weight的值
	ewt int64
	// current weight
	//当前权重，默认为0，之后会动态调整，每次选取后端时，会遍历集群中所有的后端，对于每个后端，让它的cwt增加它的ewt，同时累加所有后端的ewt，保存为total
	//如果该后端的cwt是最大的，就选定这个后端，然后把它的cwt-total，如果该后端没有被选定，那么cwt不用减小
	cwt int64
	// last score
	score float64 //根据节点权重和cpu等情况动态打分，默认为-1
}
