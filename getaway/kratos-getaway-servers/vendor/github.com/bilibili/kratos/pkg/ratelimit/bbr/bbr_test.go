...
func TestBBRs(t *testing.T) {

	g:=NewGroup(nil)
	d,err:=g.Get("sss").Allow(context.TODO())
	if err!=nil {
		fmt.Println("err=",err)
		return
	}
	d(limit.DoneInfo{Op:limit.Ignore})
	fmt.Println(limit.Success)
	fmt.Println(limit.Ignore)
	fmt.Println(limit.Drop)
...
func TestBBR(t *testing.T) {
	...
	fmt.Println("limiter = ",limiter.(*BBR).conf)
	fmt.Println("limiter = ",limiter.(*BBR).cpu())
	return
		...
		go func() {
			...
			for i := 0; i < 300; i++ {
				...
				if err != nil {
					...
					fmt.Println("&drop=",atomic.LoadInt64(&drop))
				} else {
					count := rand.Intn(100)
