...
func TestBalancerPick(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		log.Info("the subconn picked(%s),but expected(%s)", sc.addr.Addr, res[i])
	...
	for i := 0; i < 4; i++ {
		...
		log.Info("the (%d) subconn picked(%s),but expected(%s)", i, sc.addr.Addr, res[i])
	}
